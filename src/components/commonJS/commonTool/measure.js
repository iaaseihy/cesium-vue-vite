 ...
 * @LastEditTime: 2023-02-10 16:30:11
 */
...
export default class Measure {
  constructor(core) {
    ...
    this.triangleSpace = {
      ...
      createPoint(cartesian, _self) {
        const entity = _self.entitys.createEntity()
  ...
  remove() {
    ...
    if (this.areaSpace.entity.length > 0) {
      for (const i in this.areaSpace.entity) { this.entitys.remove(this.areaSpace.entity[i]) }
    }
    if (this.triangleSpace.entity.length > 0) {
      for (const i in this.triangleSpace.entity) { this.entitys.remove(this.triangleSpace.entity[i]) }
    }
    if (this.lineSpace.entity.length > 0) {
      for (const i in this.lineSpace.entity) { this.entitys.remove(this.lineSpace.entity[i]) }
    }
  }
  ...
  drawArea() {
    ...
    try {
      ...
      _self.handlerAction.Action(function(e) { // 第一次点击
        ...
        const cartesian = _self.mouseManager.screenToWorld(e.position)
        if (_self.areaSpace.positions == 0) {
          _self.areaSpace.positions.push(cartesian.clone())
        }
        _self.areaSpace.positions.push(cartesian) // 模拟

        const cartographic = Cesium.Cartographic.fromCartesian(_self.areaSpace.positions[_self.areaSpace.positions.length - 1])
  ...
  drawTriangle() {
    ...
    _self.handlerAction.Action(function(e) { // 第一次点击
      ...
      if (_self.triangleSpace.position1.length == 0) { // 开始
        const cartesian = _self.mouseManager.piTerrainToModule(e.position) // 判断拾取的坐标 地形 or 模型
    ...
    _self.handlerAction.Actions(function(e) {
      const cartesian = _self.mouseManager.piTerrainToModule(e.endPosition)
      _self.entitys.showTip(_self._resultTip, true, cartesian, '点击地图')
    }, function(e) { // 两个
      ...
      try {
        const cartesian = _self.mouseManager.piTerrainToModule(e.endPosition) // 判断拾取的坐标 地形 or 模型
        if (!cartesian) return false
