 ...
 * @LastEditTime: 2023-04-10 17:47:29
 */
/**
 * 雷达效果封装
 */
...
export default class Radar {
  ...
  createScan1() {
    ...
    _self.handler.Action(function(movement) {
      const cartesian = _self.mouseManager.pickEllipsoid(movement) // 提取lat lon画雷达
    ...
    // 移动
    _self.handler.Action(function(movement) {
      const position = _self.mouseManager.screenToWorld(movement.endPosition)
  ...
  createRadar1() {
    ...
    _self.handler.Action(function(movement) {
      const cartesian = _self.mouseManager.pickEllipsoid(movement) // 提取lat lon画雷达
      if (!cartesian) return false
      let l; let r; const lat = cartesian.y; const lon = cartesian.x; const height = cartesian.z
      // radarscan
...
class dmRadar {
  ...
  load() {
    var _self = this; var options = { camera: _self.viewer.scene.camera, canvas: _self.viewer.scene.canvas }
  ...
  customStyle() {
    var _self = this; var dataSource = this.raderData; var removeListener; var singleDigitPins = new Array(8)
