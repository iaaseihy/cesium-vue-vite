 ...
 * @LastEditTime: 2023-02-10 16:39:20
 */
/**
 * 动态绘制图形
 * 返回坐标点
...
export default class DrawDynamicTool {
  ...
  drawPoint(callback) {
    try {
      ...
      handler.setInputAction(function (movement) {
        ...
        const position = _this.mouseManager.worldToLonlat(cartesian)
  ...
  drawRect(callback) {
    ...
    handle.setInputAction(function(click) {
      ...
      if (tempPosition) {
        if (_self.shape.points.length == 0) {
          ...
          const cartesian = _self.viewer.scene.globe.ellipsoid.cartesianToCartographic(tempPosition)
    ...
    handle.setInputAction(function(movement) {
      ...
      const moveEndPosition = _self.getPointFromWindowPoint(movement.endPosition)
      if (_self.shape.points.length == 0) {
      ...
      if (moveEndPosition) {
        ...
        const cartesian = _self.viewer.scene.globe.ellipsoid.cartesianToCartographic(moveEndPosition)
