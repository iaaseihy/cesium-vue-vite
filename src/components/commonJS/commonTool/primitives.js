 ...
 * @LastEditTime: 2023-04-10 17:47:34
 */
/**
 * primitives 绘图类
 * 绘图管理
...
export default class Primitives {
  ...
  countModelMatrix(positions) {
    const lon = positions.lon; const lat = positions.lat; const length = positions.alt
    const positionOnEllipsoid = Cesium.Cartesian3.fromDegrees(lon, lat) // 1.2 地面位置(垂直地面)
    const centerOnEllipsoid = Cesium.Cartesian3.fromDegrees(lon, lat, length * 0.5) // 1.3 中心位置
    const topOnEllipsoid = Cesium.Cartesian3.fromDegrees(lon, lat, length) // 1.4 顶部位置(卫星位置)
    const modelMatrix = Cesium.Matrix4.multiplyByTranslation( // 1.5 矩阵计算
