 ...
 * @LastEditTime: 2023-02-10 11:34:38
 */
...
export default class entityFactory {
  ...
  createTriangleMeasure(poly) {
    ...
    var _update_label = function () {
      if (_self.positions.length == 1) { return false }
  ...
  createLine(opt) {
    this.options = {
      ...
      polyline: {
        ...
        clampToGround: opt.clampToGround !== undefined // 贴地
      }
    }
  ...
  /***
     ...
     */
  createScan(opt) {
    const _self = this; const viewer = opt.v; const length = opt.positions.alt
  ...
  static createLightScan(opt) {
    const viewer = opt.v; const data = opt.data
    const point = LightScanHelps.createLightScan_getCirclePoints(data.circle[0], data.circle[1], data.circle[2], data.circle[3]) // 生成分割点
    const entityCList = LightScanHelps.createLightScan_entityCList(viewer, point, data) // 生成 entityCList 圆锥
  ...
  static createLightScanFollowEntity(opt) {
    const viewer = opt.v; const data = opt.data; const model = opt.model
    const point = LightScanHelps.createLightScan_getCirclePoints(data.circle[0], data.circle[1], data.circle[2], data.circle[3]) // 生成分割点
    const entityCList = LightScanHelps.createLightScan_entityCList(viewer, point, data) // 生成 entityCList 圆锥
    viewer.scene.postRender.addEventListener(function () { // 实时获取模型的经纬度。
      ...
      if (center) {
        ...
        const X0 = lon - data.circle[1]; const Y0 = lat - data.circle[2] // 差值
...
class LightScanHelps {
  ...
  static createLightScan_entityCList(viewer, point, data) {
    const lon = data.observer[0]; const lat = data.observer[1]; const h = data.observer[2]
    const entityCList = []
    // 创建 面
  ...
  static createLightScan_changeOnePosition(data, entity, arr) {
    ...
    let x; let y; let x0; let y0; let X0; let Y0; let n = 0; let a = 0// x代表差值 x0代表差值等分后的值，X0表示每次回调改变的值。a表示回调的循环窜次数，n表示扫描的坐标个数
    function f(i) {
