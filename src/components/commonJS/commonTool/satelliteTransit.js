 ...
 * @LastEditTime: 2023-04-10 17:46:38
 */
/**
 * 卫星过境封装类
 */
...
export default class satelliteTransit {
  ...
  initST() {
    ...
    _self.handlerAction.Action(function(e) {
      try {
        ...
        const f_name = _self.pickedFeature.id.name; let f_position; let position
  ...
  loadCZML() { // load数据
    ...
    _self.CoreV.dataSources.add(Cesium.CzmlDataSource.load(config.DATADIR + '/test.czml')).then(function(dataSource) {
      ...
      const satellite1 = dataSource.entities.getById('Satellite/ISS')
      const satellite2 = dataSource.entities.getById('Satellite/Geoeye1')
      const transit1 = dataSource.entities.getById('Facility/AGI-to-Satellite/ISS')
      const transit2 = dataSource.entities.getById('Facility/AGI-to-Satellite/Geoeye1')
  ...
  communication() {
    ...
    const r_position = _self.radar.position.getValue(_self.STClock.currentTime)
    const r_point = _self.mouseManager.piEllipsoid(r_position)
    ...
    // 遍历卫星
    for (const i in _self.satelliteAll) {
      _self.p_line[i] = []
      const sate = _self.satelliteAll[i]
      const s_position = sate.position.getValue(_self.STClock.currentTime)
      ...
      const lineObj = new entityFactory({ type: 'createLine', data: { positions: _self.p_line[i], width: 1 } })
  ...
  satelliteInfo() {
    ...
    const pass_flag = function(clock) {
      try {
        if (_self.satelliteAll.length == 0) {
          return false
        }
        for (const i in _self.satelliteAll) { // 遍历卫星
          const sate = _self.satelliteAll[i]
          const position = sate.position.getValue(clock.currentTime)
          const s_point = _self.mouseManager.piEllipsoid(position)
          _self.p_line[i].pop()
          ...
          const flag = _self.pointInsideCircle(_self.ss_point, _self.rr_point, 30)
          if (flag) {
    ...
    _self.STClock.onTick.addEventListener(function(clock) {
      try {
        ...
        if (!_self.iden) {
          ...
          if (_self.pickedFeature.type == 'radar') {
            _self.pickedFeature.id.description = _self.pickedFeature.temp + _self.infoTable_1(_self.pass(clock))
          } else {
            const position = _self.pickedFeature.id.position.getValue(clock.currentTime)
            const f_position = _self.mouseManager.piEllipsoid(position)
  ...
  formatTransit() {
    ...
    _self.Tools.arrForEach(_self.Transits, function(transit, index) {
      const intervals = []; const intervals_old = []
      const n_interval = transit.availability._intervals
      _self.Tools.arrForEach(n_interval, function(interval, index) {
        const start = dayjs(Cesium.JulianDate.addHours(interval.start, -8, new Cesium.JulianDate())).format('YYYY-MM-DD HH:mm:ss')
        const stop = dayjs(Cesium.JulianDate.addHours(interval.stop, -8, new Cesium.JulianDate())).format('YYYY-MM-DD HH:mm:ss')
  ...
  createEntity(radars) {
    try {
      ...
      _self.Tools.arrForEach(radars, function(radar) {
        let l, r
        const positions = radar.position.getValue(_self.STClock.currentTime)
        _self.r_point = positions
        if (positions.length == 0) {
          return false
        };
        const cartographic = _self.mouseManager.piEllipsoid(positions)
        const lat = Cesium.Math.toDegrees(cartographic.latitude); const lon = Cesium.Math.toDegrees(cartographic.longitude); const height = cartographic.height
        // radarscan
  ...
  scan() {
    ...
    _self.Tools.arrForEach(_self.satelliteAll, function(entity, index) {
      ...
      const positions = _self.mouseManager.worldToLonlat(cartesian)
      _self.bindScan(positions, entity)
    })
  }

  pass(clock) { // 当前时间
    const currentTime = dayjs(Cesium.JulianDate.addHours(clock.currentTime, -8, new Cesium.JulianDate())).format('YYYY-MM-DD HH:mm:ss')
    return currentTime
  }
  ...
  infoTable_1(currentTime) {
    try {
      const _self = this; let renderTime = _self.renderTime
  ...
  infoTable_2(f_name, cartesian) {
    ...
    let tr = ''; const table = '<h2> Position </h2><table class="cesium-infoBox-defaultTable"><thead><tr><th>Name</th><th>Latitude</th><th>Longitude</th><th>Elevation</th></tr></thead><tbody>'
    const f_point = [parseInt(cartesian.longitude / Math.PI * 180), parseInt(cartesian.latitude / Math.PI * 180)]
