...
export default class FlyPath {
  ...
  /**
     ...
     */
  run() {
    const _self = this; let orientation
    const property = new Cesium.SampledPositionProperty() // 属性
    _self.Tools.arrForEach(_self.mFlyPath, function(vue, index) { // value index
      const time = Cesium.JulianDate.addSeconds(_self.startTime, vue.time, new Cesium.JulianDate())
      const position = Cesium.Cartesian3.fromDegrees(vue.longitude, vue.dimension, vue.height)
      property.addSample(time, position) // 添加位置，和时间对应
    })
    const entity = _self.entitys.createEntity() // 创建一个实体
    // 给实体赋值
    entity.name = '无人机'
  ...
  flyInfo() {
    ...
    _self.handlerAction.Action(function(e) { // 单击
      ...
      const f_position = _self.mouseManager.piEllipsoid(_self.pickedFeature.id.position.getValue(_self.flyClock.currentTime))
    ...
    // 分析工具 和 动画
    const infoBoxHtml = `<div id="btn_cekong"><button type="button" title="地面分析" class="analysisGround" id="btn_ckexport" style="position:absolute;left:35px;top:3px;background:rgb(84, 84, 84);border:0px;"><i class="fa fa-upload "></i></button>
  ...
  infoTable(f_name, cartesian) {
    ...
    let tr = ''; const table = '<h2> 位置点 </h2><table class="cesium-infoBox-defaultTable"><thead><tr><th>Name</th><th>Latitude</th><th>Longitude</th><th>Elevation</th></tr></thead><tbody>'
  ...
  scanInfo(position) {
    ...
    let tr = ''; const table = '<table class="cesium-infoBox-defaultTable"><thead><tr><th>类别</th><th>经度</th><th>纬度</th><th>总数</th></tr></thead><tbody>'
    const f_point = [parseInt(position.longitude / Math.PI * 180), parseInt(position.latitude / Math.PI * 180)]
  ...
  /**
     ...
     */
  analysisGround(doc) {
    const analysisGround = doc.getElementsByClassName('analysisGround')[0]
  ...
  customFly() {
    ...
    _self.handlerAction.Action(function(e) {
      ...
      const cartesian = _self.mouseManager.piScreen(e.position) // 拾取屏幕坐标

      if (_self.positions.length == 0)_self.positions.push(cartesian) // 第一次创建保证有两个点
      _self.positions.push(cartesian)

      // 飞行路径实体
      const lonlat = _self.mouseManager.screenToLonlat(e.position) // 屏幕坐标转经纬度 高程
    ...
    _self.handlerAction.Actions(function() {}, function(e) {
      ...
      const cartesian = _self.mouseManager.piScreen(e.endPosition) // 拾取屏幕坐标
