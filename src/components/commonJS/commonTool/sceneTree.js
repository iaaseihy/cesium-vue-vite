 ...
 * @LastEditTime: 2023-04-10 17:45:12
 */
/**
 * @file sceneTree
 * @version 1.0
...
export default class sceneTree {
  ...
  build(trees) {
    ...
    if (trees.scenes) {
      ...
      const createScene = function(scene) {
        ...
        if (scene.type == 'model') {
          ...
          if (_self.Tools.strBool(scene.url)) {
            ...
            model = _self.modelManager.createDefault({
              id: trees._model.id,
              url: trees.scene.url,
              position:
                            Cesium.Cartesian3.fromDegrees(scene.position[0], scene.position[1].dimension, 0)
      ...
      // 遍历场景
      for (const key in trees.scenes) {
        const scene = trees.scenes[key]
        if (scene !== undefined) { createScene(scene) }
      }
    }
    // 加载地形
    if (trees.terrain) {
