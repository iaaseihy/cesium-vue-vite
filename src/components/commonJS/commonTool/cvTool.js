 ...
 * @LastEditTime: 2023-02-10 11:34:10
 */
/**
 *
 * Cvearth Tools封装
...
export default class CVTools {
  ...
  saveJsonText(j) {
    const content = JSON.stringify(j.jsonData)
  ...
  nullBool(param) { // 对象为空判断 为空返回true，反之false
    if (param === null || undefined === param) return true
    return false
  }
  ...
  arrBool(param) { // arraty判断 length不为零 为空返回true，反之false
    if (param === null || undefined === param || param.length === 0) return true
    return false
  }

  mapBool(param) { // 判断map是否为空 为空返回true，反之false
    for (var t in param) { return !1 }
    return !0
  }
  ...
  isChildren(contrastObj, boolObj) { // 判断是否是指定对象子类或自身 ， 是返回true
    ...
    if (!this.nullBool(contrastObj) && !this.nullBool(boolObj)) {
      if (boolObj[0].nodeName.toLowerCase() === 'body') {
        result = false
      } else {
