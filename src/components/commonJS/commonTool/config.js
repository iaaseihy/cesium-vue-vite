...
export default class CONFIG {
  constructor() {}
    ...
    static createSceneTree(viewer) {
      const sceneTree = new SceneTree(viewer)
      const Options = function () {
        this.fog = true
        this.lighting = false
    ...
    static initMouseInfo(viewer) {
      const handler = new Handler(viewer); const mouseManager = new MouseManager(viewer)
      ...
      handler.Action(function (event) {
        ...
        const latlonObj = mouseManager.piTerrainToModule(event.endPosition, '1')
        // 修改鼠标位置
