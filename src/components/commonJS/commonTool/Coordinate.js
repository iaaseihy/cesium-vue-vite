...
export default class CoordinateHelper extends mouseManager {
  ...
  addHieght_cartesian(cartesian, height) {
    const tempCoord = this.cartesian2lonlat(cartesian)
    tempCoord.height += height
    const car = this.lonlat2cartesian(tempCoord.longitude, tempCoord.latitude, tempCoord.height)
    return car
  }
  ...
  decreaseHeight(cartesianArray) {
    let minHeight
    const lonAndLatArray = cartesianArray.map((value) => {
