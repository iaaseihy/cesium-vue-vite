import * as satellitejs from './satellite.es.js'
  ...
  computePasses(groundStation,
    ...
    while (date < endDate) {
      try {
        ...
        const distance = this.dataDistance({ x: positionGeo.longitude, y: positionGeo.latitude, z: positionGeo.height }, { x: groundStation.longitude, y: groundStation.latitude, z: groundStation.height }) // 距离
        if (distance) {
          // 经过
