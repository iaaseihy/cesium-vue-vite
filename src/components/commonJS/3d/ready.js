 ...
 * @LastEditTime: 2023-04-11 14:44:13
 */
/**
 * @file ready
 * @version 1.0
...
import Tools from '../commonTool/cvTool.js'
...
export default class Read {
    ...
    onUseA(call) {
      const pro1 = new Promise((resolve, reject) => {
        ...
        const cesium = this.Html.loadJS(config.APPDIR + '/Build/Cesium/Cesium.js')
        cesium.onload = function() { resolve('-- loadCesium --') }
      })
      pro1.then((res) => {
        setTimeout(() => {
          const pro2 = new Promise(this.loadUse)
          pro2.then((res) => {
            console.log(res)
            call(res)
          })
