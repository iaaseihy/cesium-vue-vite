...
Components.TAG = {
  ...
  searchRegionBox: {
    ...
    js: function(callback) {
      // 区域搜索
      const singleSelect1 = $('#single-select-1').citySelect({
        dataJson: cityData,
        multiSelect: false,
        whole: true,
        shorthand: true,
        ...
        onCallerAfter: function (target, values) {
          const temp_url = 'http://localhost:8080/geoserver/zt/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zt%3Acounty2019&outputFormat=application%2Fjson&CQL_FILTER=admincode=' + values.id
