...
var WXTX = (function() {
  // 全局的属性
  let viewer; let radar; let satelliteAll = []
  let transit = []; let selectedEntity
  let handle = null; let renderTime; let passTime = []
  let pass_old = []; let iden = true
  let pickedFeature = null; const num_pass = 0
  const radarEntity = []; const _entity = []
  let data = null; let t = null
  ...
  _.prototype.destroy = function() {
    ...
    for (const i in _entity)viewer.entities.remove(_entity[i])
  ...
  _.prototype.getPoint = function(positions) {
    ...
    var hei = parseFloat(cartographic.height / 4)
...
var fly = (function() {
  ...
  _.prototype.create = function(data) {
    ...
    // 结束时间
    const stop = Cesium.JulianDate.addSeconds(start, (data[0].length - 1) * 120, new Cesium.JulianDate())
    // 设置始时钟始时间
    ...
    for (let j = 0; j < data.length; j++) {
      ...
      for (let i = 0; i < data[j].length; i++) {
        const time = Cesium.JulianDate.addSeconds(start, data[j][i].time, new Cesium.JulianDate())
        const position = Cesium.Cartesian3.fromDegrees(data[j][i].longitude, data[j][i].dimension, data[j][i].height)
        // 添加位置，和时间对应
...
var drawTerrain = (function () {
  ...
  _.prototype = {
    ...
    remove: function () {
      if (viewer != undefined) {
        for (const i in e) viewer.entities.remove(e[i])
  ...
  _.prototype = {
    ...
    remove: function () {
      if (viewer != undefined) {
        ...
        for (const i in t.floatingPoint) viewer.entities.remove(t.floatingPoint[i])
...
var TileLonlatController = (function() {
  var viewer; var layer = null; var tableparam; var layers = { grid: { data: null }, default: { data: null }, xmlData: { data: null } }
  function _() {};
  _.init = function (param, flag) {
    if (param === null || undefined === param) return
    var t = this
  ...
  _.controller = function(obj) {
    var _self = this; var key = obj.key; var value = parseFloat(obj.value)
...
var CesiumRadar1 = (function() {
  var viewer; var _raderData = null
  function _() {};
  ...
  _.config = function() {
    ...
    var t = this; var dataSource = this._raderData
    var viewModel = {
  ...
  _.customStyle = function() {
    var t = this; var dataSource = this._raderData; var removeListener; var singleDigitPins = new Array(8)
  ...
  _.loadKml = function(param) {
    var t = this; var kml = param.kml; var options = { camera: t.viewer.scene.camera, canvas: t.viewer.scene.canvas }
...
WindField.prototype = {
  ...
  _bilinearInterpolation: function (x, y, g00, g10, g01, g11) {
    var rx = (1 - x)
    var ry = (1 - y)
    var a = rx * ry; var b = x * ry; var c = rx * y; var d = x * y
...
Windy.prototype = {
  ...
  _createLineInstance: function (positions, ageRate) {
    var colors = []; var _color = this.color
...
var CesiumHeatMap = (function () {
  var viewer; var heatmap = null; var cacheData = null; var tableparam
  function _() {}
  _.init = function(param, flag) {
  ...
  _.click = function() {
    var _self = this; var handler = new Cesium.ScreenSpaceEventHandler(_self.viewer.scene.canvas)
