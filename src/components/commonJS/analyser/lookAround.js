 ...
 * @LastEditTime: 2023-02-10 16:51:24
 */
...
import DrawDynamicTool from '../commonTool/DrawDynamicTool'
export default class LookAround extends Analyser {
  ...
  analyser(cartesian, r) { // viewHeight
    try {
      ...
      const _self = this; const viewHeight = 10
      ...
      const ellipse = CesiumBuild.EllipseGeometryLibraryEx.computeEllipseEdgePositions(eopt) // 范围当前椭圆位置的数组
      for (let i = 0; i < ellipse.outerPositions.length; i += 3) {
        // 逐条计算可视域
        const cartesian = new Cesium.Cartesian3(ellipse.outerPositions[i], ellipse.outerPositions[i + 1], ellipse.outerPositions[i + 2])
        const cartographic = Cesium.Cartographic.fromCartesian(cartesian)
        const deltaRadian = 0.00005 * Math.PI / 180.0 // Cesium.Math.RADIANS_PER_DEGREE
        const cartographicArr = CesiumBuild.SysMathTool.InterpolateLineCartographic(cartographicCenter, cartographic, deltaRadian)
        CesiumBuild.ThreeDTilesToolCopy.CartographicPointsTerrainData(_self._viewer, cartographicArr,
          function (terrainData) {
            if (terrainData.length > 0) {
              let preVisible = true
              const cartesiansLine = []
              const colors = []
              for (let j = 1; j < terrainData.length; j++) {
                // 逐点计算可见性
                let visible = true// 该点可见性
                if (j > 1) {
                  const cartographicCenterHV = new Cesium.Cartographic(terrainData[0].longitude, terrainData[0].latitude, terrainData[0].height + viewHeight)
                  console.log(cartographicCenterHV)
                  if (preVisible) {
                    //
                    const curPoint = CesiumBuild.SysMathTool.InterpolateIndexLineHeightCartographic(cartographicCenterHV, terrainData[j], j, j - 1)
                    ...
                    // 插值到当前
                    const curPointArr = CesiumBuild.SysMathTool.Interpolate2IndexLineHeightCartographic(cartographicCenterHV, terrainData[j], j, j - 1)
                ...
                const cartesianTemp = Cesium.Cartesian3.fromRadians(terrainData[j].longitude, terrainData[j].latitude, terrainData[j].height + 1)
              ...
              // 绘制结果
              const pointsKSY = new CesiumBuild.PrimitivePoints({ viewer: _self._viewer, Cartesians: cartesiansLine, Colors: colors })
