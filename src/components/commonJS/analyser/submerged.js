 ...
 * @LastEditTime: 2023-04-11 14:45:43
 */
...
export default class Submerged extends Analyser {
  ...
  analyser(options) {
    /**
         * 点击
         */
    const _self = this; const viewer = this._viewer; let info = ''
		 const maxH = options.maxH === undefined ? 50 : options.maxH
		 const interval = options.interval === undefined ? 10 : options.interval
    ...
    _self.handler.setInputAction(function (e) { // 第一次点击
      ...
      const cartographic = Cesium.Cartographic.fromCartesian(_self.positions[_self.positions.length - 1])
