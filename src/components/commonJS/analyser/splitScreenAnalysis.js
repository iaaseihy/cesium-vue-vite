...
SplitScreenAnalysis.prototype.viewerLloadModel = function (options) {
  ...
  const Cesium = this._Cesium
  const model = this._viewerL.scene.primitives.add(Cesium.Model.fromGltf({
    // 资源路径
    url: options.url,
    // 模型矩阵
...
SplitScreenAnalysis.prototype.viewerRloadModel = function (options) {
  ...
  const Cesium = this._Cesium
  const model = this._viewerR.scene.primitives.add(Cesium.Model.fromGltf({
    // 资源路径
    url: options.url,
    // 模型矩阵
    modelMatrix: options.modelMatrix,
...
SplitScreenAnalysis.prototype.syncViewer = function () {
  const that = this
  const Cesium = this._Cesium
  const viewerL = this._viewerL
  const viewerR = this._viewerR
