...
<template>
    ...
    <div class="demo-container">
      ...
      <div class="ClipPlane">
            <el-button @click="createClipPlanAnalysis(viewer)">地形开挖分析</el-button>
      </div>
      <div class="offClipPlane">
            <el-button @click="clearClipPlanAnalysis()">关闭地形开挖分析</el-button>
      </div>
   ...
   */
    ...
    transformCartesianArrayToWGS84Array: function (viewer, cartesianArr) {
      if (viewer) {
        var $this = this
        return cartesianArr
          ? cartesianArr.map(function (item) { return $this.transformCartesianToWGS84(viewer, item) })
          : []
      }
    },
    // 坐标转换 笛卡尔转84
    ...
    transformWGS84ArrayToCartesianArray: function (viewer, WSG84Arr, alt) {
      if (viewer && WSG84Arr) {
        var $this = this
        return WSG84Arr
          ? WSG84Arr.map(function (item) { return $this.transformWGS84ToCartesian(viewer, item, alt) })
          : []
      }
    },
    /**
      ...
      if (viewer && px) {
        ...
        if (isOn3dtiles) {
          ...
          if (cartesian) {
            ...
            cartesian = this.transformWGS84ToCartesian(viewer, { lng: lon, lat: lat, alt: height })
          }
        }
        // 地形
    ...
    drawPolygonGraphics: function (options, viewer) {
      ...
      if (viewer && options) {
        ...
        // mouse
        _handler.setInputAction(function (movement) {
          var cartesian = $this.getCatesian3FromPX(movement.endPosition, viewer)
        ...
        _handler.setInputAction(function (movement) {
          ...
          positions.push(positions[0])
          const positionsgeo = $this.transformCartesian3ArrayToWGS84Array(viewer, positions)
          ...
          if (typeof options.callback === 'function') {
            options.callback($this.transformCartesianArrayToWGS84Array(viewer, positions), polyObj)
          }
        ...
        function _addInfoPoint(position) {
          ...
          _labelEntity.label = {
            text: ($this.getPositionsArea($this.transformCartesianArrayToWGS84Array(viewer, positions)) / 1000000.0).toFixed(4) + '平方公里',
            show: true,
            showBackground: true,
            font: '14px monospace',
    ...
    createClipPlanAnalysis: function (viewer) {
      ...
      if (viewer && options) {
        ...
        var _bottomImg = options.bottomImg || '/static/texture/excavate_bottom_min.jpg'
        $this.drawPolygonGraphics({
          callback: function (polygon, polygonObj) {
            ...
            $this.terrainClipPlan.updateData($this.transformWGS84ArrayToCartesianArray(viewer, polygon))
    ...
    terrainClipPlanFunction(viewer) {
      ...
      handler.setInputAction((movement) => {
        ...
        const cartographic = Cesium.Cartographic.fromCartesian(cartesian, this.viewer.scene.globe.ellipsoid, new Cesium.Cartographic())
