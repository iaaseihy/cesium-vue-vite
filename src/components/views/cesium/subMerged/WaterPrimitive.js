...
uniform sampler2D textureReflect; // 反射贴图
...
czm_material czm_getMaterial(czm_materialInput materialInput) {
    ...
    vec3 reflectionSample = vec3( texture( textureReflect, (v_uv.xy / v_uv.w) * 0.5 + 0.5 + distortion ) );
...
out vec3 position3DHigh;
out vec3 position3DLow;
out vec3 normal;
out vec2 st;
out float batchId;
...
export default class WaterPrimitive {
  ...
  _createFramebuffer(context, width, height, hdr) {
    ...
    const pixelDatatype = hdr
      ...
      // 检查最大纹理尺寸
var maxTextureSize = Cesium.ContextLimits.maximumTextureSize;
console.log(maxTextureSize);
var width = Math.min(width, maxTextureSize);
var height = Math.min(height, maxTextureSize);
